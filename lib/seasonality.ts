...
export type SeasonContext = {
  ...
  label: string;
  /**
   * How much market-wide CTR baseline deviates from the annual mean.
   * 1.0 = normal, 1.3 = 30% above baseline (Q4 peak).
   */
  ctrMultiplier: number;
  atcMultiplier: number;
  convMultiplier: number;
  /** CPM rises in Q4 due to auction pressure. */
  cpmMultiplier: number;
  /** Points added to raw confidence score (-20..+15). */
  confidenceBonus: number;
  /**
   * -1..+1. Positive = more willing to upgrade TEST AGAIN → SCALE.
   * Negative = more cautious (post-holiday slump).
   */
  scaleEagerness: number;
  /**
   * 0..1. How strongly to suppress KILL verdicts.
   * 0.5+ = require truly zero ATC before killing (peak season).
   */
  killReticence: number;
  /** Whether the effect is large enough to surface in verdict text. */
  isMaterial: boolean;
  note: string | null;
};

/**
 * ISO 8601 week-of-year (W01 = week containing Jan 4th).
 * Ranges 1-53.
 */
...
export function getSeasonContext(date: Date = new Date()): SeasonContext {
  ...
  // Black Friday / Cyber Monday — W47 (third week of November)
  if (week === 47) {
    return {
  ...
  // Q4 pre-peak — W43-W50 (mid-Oct through late Nov, excluding BF week)
  if (week >= 43 && week <= 50) {
    return {
      month, weekOfYear: week,
      label: "Q4 Pre-peak",
  ...
  // Post-holiday slump — W1-W3 (Jan 1-20)
  if (week <= 3) {
    return {
      month, weekOfYear: week,
  ...
  // Back to school — W33-W38 (takes priority over late summer)
  if (week >= 33 && week <= 38) {
    return {
      month, weekOfYear: week,
