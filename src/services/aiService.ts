...
async function getAccessToken(): Promise<string> {
  ...
  const res = await httpsPost(
    ...
    `scope=${encodeURIComponent(env.GIGACHAT_SCOPE)}`,
  );
  ...
  const data = JSON.parse(res.text) as { access_token: string; expires_at: number };
...
export async function completeGigaChatJson<T>(
  ...
  const res = await httpsPost(
    `${env.GIGACHAT_BASE_URL}/chat/completions`,
    {
  ...
  if (!content) throw new Error(`Empty GigaChat response for ${schemaName}`);
...
export type AiAnalysisResult = {
  ...
  creativeAngles: Array<{ hookIdea: string; concept: string; targetEmotion: string }>;
  continueDecision: {
...
export async function runAiAnalysis(input: AnalysisPayload): Promise<AiAnalysisResult> {
  ...
  const [decision, diagnosis, actionPlan, validation, funnelLeak, creativeAngles, continueDecision] =
    await Promise.all([
      complete<AiAnalysisResult["decision"]>(
        "decision",
        SYSTEM,
        `Analyze this ad campaign and return one strict decision.
...
${dataBlock}`,
      ...
      complete<AiAnalysisResult["diagnosis"]>(
        "diagnosis",
        SYSTEM,
        `Identify the single main performance problem.
...
${dataBlock}`,
      ...
      complete<{ actionPlan: string[] }>(
        "actionPlan",
        SYSTEM,
        `Give exactly 3 specific executable next actions. No generic advice.
...
${dataBlock}`,
      ...
      complete<AiAnalysisResult["validation"]>(
        "validation",
        SYSTEM,
        `Evaluate if this product has real market potential.
...
${dataBlock}`,
      ...
      complete<AiAnalysisResult["funnelLeak"]>(
        "funnelLeak",
        SYSTEM,
        `Find the weakest stage: "impressions → clicks", "clicks → add to cart", or "add to cart → purchase".
...
${dataBlock}`,
      ...
      complete<{ creativeAngles: Array<{ hookIdea: string; concept: string; targetEmotion: string }> }>(
        "creativeAngles",
        SYSTEM,
        `Generate 3 direct-response ad angles for this product. No generic ideas.
...
Context: CTR ${input.ctr}%, ${modelInput.purchases} purchases from ${input.clicks} clicks`,
      ...
      complete<AiAnalysisResult["continueDecision"]>(
        "continueDecision",
        SYSTEM,
        `Decide whether to continue testing or stop this campaign.
...
${dataBlock}`,
  ...
  return {
    decision,
    diagnosis,
    actionPlan: ((actionPlan as { actionPlan: string[] }).actionPlan ?? []).slice(0, 3),
    validation,
    profitability: {
