...
export const scenarioInputSchema = z.object({
  ...
  changes: z.object({
    ctr_delta_pct: z.number().optional(),
    conversion_delta_pct: z.number().optional(),
    cpc_delta_pct: z.number().optional(),
    aov_delta_pct: z.number().optional(),
    new_budget: z.number().positive().optional(),
  }),
});
...
function deltaPct(base: number, projected: number): number {
  if (base === 0) return 0;
...
export function runScenario(input: ScenarioInput): ScenarioResult {
  ...
  const profitPct =
    baseMetrics.profit !== 0 ? deltaPct(baseMetrics.profit, projected.profit) : null;
