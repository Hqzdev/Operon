...
exports.analyzeStore = analyzeStore;
...
function fallbackStoreAnalysis(storeUrl, platform, meta) {
...
async function analyzeStore(storeUrl) {
    let html = "";
...
Suggested metrics should be specific KPIs relevant to this store's niche (e.g. 'ROAS', 'Cart Abandonment Rate', 'Average Order Value', 'Customer Lifetime Value', 'Return Rate', 'Email CTR').`;
    let parsed;
    try {
        parsed = await (0, aiService_1.completeGigaChatJson)("storeAnalysis", "You are a strict e-commerce analyst. Return valid JSON only. No markdown. No text outside the JSON.", prompt);
    }
    catch (error) {
