...
async function disconnectConnection(userId, connectionId) {
    await prisma_1.prisma.integrationConnection.updateMany({
        where: { id: connectionId, userId },
        data: { status: client_1.IntegrationStatus.DISCONNECTED },
    });
}
async function syncConnection(connectionId) {
    const connection = await prisma_1.prisma.integrationConnection.findUnique({ where: { id: connectionId } });
    ...
    if (connection.syncLockUntil && connection.syncLockUntil > now)
        return null;
    await prisma_1.prisma.integrationConnection.update({
    ...
    try {
        ...
        for (const metrics of raw) {
            ...
            await prisma_1.prisma.integrationMetricSnapshot.upsert({
                where: {
...
async function refreshIfNeeded(connection) {
    ...
    if (!connection.refreshToken) {
        await prisma_1.prisma.integrationConnection.update({
            where: { id: connection.id },
            data: { status: client_1.IntegrationStatus.ACTION_REQUIRED, lastError: "Token expired; reconnect required" },
        });
    ...
    if (connection.provider === client_1.IntegrationProvider.TIKTOK) {
        ...
        return prisma_1.prisma.integrationConnection.update({
            where: { id: connection.id },
            data: {
...
async function syncUserConnections(userId) {
    const connections = await prisma_1.prisma.integrationConnection.findMany({
        where: { userId, status: { not: client_1.IntegrationStatus.DISCONNECTED } },
    });
    const results = [];
...
async function getLatestAnalysisInput(userId) {
    const latest = await prisma_1.prisma.integrationMetricSnapshot.findFirst({
        where: { userId },
        orderBy: [{ date: "desc" }, { updatedAt: "desc" }],
    });
    if (!latest)
